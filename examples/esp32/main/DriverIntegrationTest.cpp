...
static bool test_polling_rotation_vector() noexcept {
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        if (g_imu->hasNewData(BNO085Sensor::RotationVector)) {
            ...
            ESP_LOGI(TAG, "Rotation Vector: w=%.3f, x=%.3f, y=%.3f, z=%.3f, accuracy=%u",
                     rot.rotation.w, rot.rotation.x, rot.rotation.y, rot.rotation.z,
                     rot.rotation.accuracy);
            data_received = true;
            break;
        }
...
static bool test_sensor_data_reading() noexcept {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        if (g_imu->hasNewData(BNO085Sensor::Gravity)) {
            ...
            ESP_LOGI(TAG, "Gravity: x=%.3f, y=%.3f, z=%.3f m/s^2",
                     grav.vector.x, grav.vector.y, grav.vector.z);
        }
...
extern "C" void app_main(void) {
    ...
    RUN_TEST_SECTION_IF_ENABLED(ENABLE_SENSOR_ENABLE_TESTS, "SENSOR ENABLE/DISABLE TESTS",
    ...
    RUN_TEST_SECTION_IF_ENABLED(ENABLE_POLLING_MODE_TESTS, "POLLING MODE TESTS",
        RUN_TEST_IN_TASK("test_polling_rotation_vector", test_polling_rotation_vector, 8192, 5);
        RUN_TEST_IN_TASK("test_polling_linear_acceleration", test_polling_linear_acceleration, 8192, 5);
    );
    ...
    RUN_TEST_SECTION_IF_ENABLED(ENABLE_SENSOR_DATA_TESTS, "SENSOR DATA TESTS",
