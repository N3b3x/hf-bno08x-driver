...
void printDsfHeaders(void) {
  printf("+0 "
         "TIME[x]{us},SAMPLE_ID[x]{samples},EULER[xyz]{deg},LIN_ACC_GRAVITY[xyz]{g},MOTION_INTENT["
         "x]{state},MOTION_REQUEST[x]{state}\n");
...
void printDsf(const rvc_SensorEvent_t* event) {
  ...
  printf(".%d %0.6f,%u,%0.2f,%0.2f,%0.2f,%0.3f,%0.3f,%0.3f,%d,%d\n",
         0,               // using sensor id 0
         t,               // time
         lastSequence,    // sequence
         value.pitch_deg, // euler angles [deg]
         value.roll_deg, value.yaw_deg,
...
void printEvent(const rvc_SensorEvent_t* event) {
  ...
  printf("%3d : yaw:%0.2f pitch:%0.2f roll:%0.2f ax:%0.3f ay:%0.3f az:%0.3f mi:%d mr:%d\n",
         value.index, value.yaw_deg, value.pitch_deg, value.roll_deg, value.acc_x_g, value.acc_y_g,
         value.acc_z_g, value.mi, value.mr);
}
