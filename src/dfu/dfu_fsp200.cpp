...
static DfuState_t handleUpgradeStatusResponse(uint8_t *payload, uint16_t len) {
  ...
  // Make sure transition to upgrade mode succeeded
  if ((opMode == OPMODE_UPGRADE) && ((status & 0x0FFFFFFF) == STATUS_UPGRADE_STARTED) &&
      (error == 0)) {
    dfu_.wordOffset = 0;
    requestWrite();
...
static DfuState_t handleFinalStatus(uint8_t *payload, uint16_t len) {
  ...
  if ((status & STATUS_APP_VALID) && ((status & STATUS_ERROR) == 0) && (errCode == DFU_NO_ERROR)) {
    requestLaunch();
    dfu_.status = SH2_OK;
...
int dfu(IDfuTransport &transport, const HcBin_t &fw) {
  ...
  start_us = now_us;
  while (((now_us - start_us) < DFU_TIMEOUT_US) && (dfu_.state != ST_FINISHED)) {
    DfuState_t state = dfu_.state;
    uint32_t maxTime_us = dfuStates[state].maxTime_us;

    if ((maxTime_us > 0) && (maxTime_us <= (now_us - dfu_.intervalStart_us))) {
